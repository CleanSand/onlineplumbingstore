...
const Basket = observer(() => {
  ...
  function plus(e, key) {
    ...
    if (amount >= 100) return (document.getElementById(key).innerHTML = amount.toString());
  ...
  function minus(e, key) {
    const amount = +document.getElementById(key).innerHTML;
    if (amount <= 1) return (document.getElementById(key).innerHTML = amount.toString());
  ...
  function btnDeleteFromBasket(e, key) {
    ...
    deleteFromBasket(formData).then(() => {
      // Удаление товара из состояния product.basket
      const updatedBasket = product.basket.filter(item => item.IDProduct_Product.IDProduct !== key);
      product.setBasket(updatedBasket);
    });
  }
        ...
        {product.basket.length === 0 ? (
            ...
            <Container className={'d-flex justify-content-between'}>
                    ...
                    <div className="m-3" key={basket.IDProduct_Product.IDProduct}>
                      <div className="card" style={{ width: '16rem', height: '100%' }}>
                        <Image
                            ...
                            src={process.env.REACT_APP_API_URL + basket.IDProduct_Product.Image}
                            alt="Card image cap"
                        />
                        <div className="d-flex flex-column justify-content-between card-body">
                          ...
                          <div className="btn-group" role="group" aria-label="Basic example">
                            ...
                            <button
                                ...
                                disabled={true}
                                type="button"
                                className="btn btn-secondary quantity"
                            >
                              {basket.Quantity}
                            </button>
                            <button onClick={e => plus(e, basket.IDProduct)} type="button" data-key={basket.IDProduct} className="btn btn-secondary">
                              +
                            </button>
                          </div>
                          <button type="button" onClick={e => btnDeleteFromBasket(e, basket.IDProduct)} className="mt-3 btn btn-danger">
                            Удалить
                          </button>
                        </div>
                      </div>
              ...
              <div className={'mt-3'} style={{ width: '30%' }}>
                <div className="card">
                  <div className="card-header">Оплата заказа</div>
                  <div className="card-body">
                    <h5 id={'totalCost'} className="card-title">
                      Стоимость: {totalCost} ₽
                    </h5>
